...
def DISPLAYRESOURCE():
    ...
    try:
        ...
        creation_date = heading_line[0:10] #[0:10] reads first 10 characters (i.e. creation date). Can also be [:10] which reads from the start till the 10th character
        num_records = heading_line[10:] #[10:] reads characters from the 10th character onwards
        ...
        #display subheadings
        print("{0:13s}{1:17s}{2:32s}{3:}".format("Resource No", "Resource Type", "Title", "Date Acquired"))
        ...
        for record_line in detail_lines:
            ...
            date_acquired = time.strptime(date_acquired, "%d%m%y") #strptime reads a string, and divides it into day, month, and year
            date_acquired = time.strftime("%d-%m-%Y", date_acquired) #strftime reads a string and applies the desired format to what was previously specified
